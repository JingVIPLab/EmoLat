...
class EmoAdjDataset_content(Dataset):
    def __init__(self, img_root, json_root, c_proportion, proportion, is_train, transform):
        # 0.比例
        self.c_proportion = c_proportion
        ...
        for emo_cls in emo_class:
            ...
            content_index = int(len(json_cls_file) * self.c_proportion)
            split_index = int(content_index * self.proportion)
            train_cls_set_file = json_cls_file[:content_index][:split_index]
            test_cls_set_file = json_cls_file[:content_index][split_index:]
