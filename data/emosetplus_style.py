...
class EmoAdjDataset_style(Dataset):
    def __init__(self, img_root, json_root, s_proportion, proportion, is_train, transform):
        # 0.比例
        ...
        for emo_cls in emo_class:
            ...
            style_index = int(len(json_cls_file) * self.s_proportion)
            split_index = int(style_index * self.proportion)
            train_cls_set_file = json_cls_file[style_index:][:split_index]
            test_cls_set_file = json_cls_file[style_index:][split_index:]
    ...
    def __getitem__(self, idx):
        ...
        return json_path, image, emo, adj, pair, img_path