...
def set_args():
    ...
    parser.add_argument('--content_img_output', default='/home/zjx/lab/my3_lab/v1/sample_img_path',help='Directory to save the model')
    # parser.add_argument('--save_net_dir', default='/home/zjx/lab/my3_lab/v1/saved_nerwork',help='Directory to save the model')
...
def test():
    ...
    if args.mode == 's':
        ...
        with torch.no_grad():
            output= network(content, args.text, index)
        ...
        for i in tqdm(range(dataloader_len)):
            ...
            with torch.no_grad():
                ...
                for i in range(5):
                    ...
                    index = index + 20
                    output = network(content_image, target, index)
                    output = output * std_s + mean_s
                    output = output.to(device)
                    output_dir = args.content_img_output
                    output_path = os.path.join(output_dir, f'target_{target}_{i}_{file_name}')
                    # output = output * std_s + mean_s
                    # output = output.cpu()
                    save_image(output, output_path)
